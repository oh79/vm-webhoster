...
class ProxyService:
    ...
    def validate_nginx_config(self) -> bool:
        ...
        """
        try:
            cmd = [str(self.manager_script), "validate"]
    ...
    def _reload_nginx(self) -> None:
        """Nginx 설정 리로드"""
        try:
            # 개발 환경에서는 nginx 리로드를 비활성화
            if settings.DEBUG:
                logger.info("개발 환경에서 Nginx 리로드 건너뜀")
                return
            
            cmd = [str(self.manager_script), "reload"]
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
